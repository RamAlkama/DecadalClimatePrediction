...
season='ONDJFM' #'AMJJAS' #'ONDJFM' # 'MAM'  'JJA'  'SON'  'DJF'  'DJFM' 'ONDJFM' 'AMJJAS' 'monthly'
refyrs='1981-2010'
...
def lims(tabs):
    ...
    YR=[]
    for tab in tabs:
        YR.append(int(tab[1:5]))
        RR.append(int(tab.split('r')[1].split('i')[0]))
        II.append(int(tab.split('r')[1].split('i')[1].split('p')[0]))
        PP.append(int(tab.split('r')[1].split('i')[1].split('p')[1].split('f')[0]))
        FF.append(int(tab.split('r')[1].split('i')[1].split('p')[1].split('f')[1]))
    return min(RR),max(RR),min(YR),max(YR),min(II),max(II),min(PP),max(PP),min(FF),max(FF)

def sort_key(version):
    if version == "latest":
        return (2, None)  
    match = re.match(r'v(\d{8})$', version)
    if match:
...
listModel =['MPI-M','CMCC', 'NCAR', 'NCC','CCCma', 'CAS', 'BCC',  'DWD', 'MIROC',  'MOHC',  'IPSL','EC-Earth-Consortium']


versionList = { 'BCC':['BCC-CSM2-MR'],  'CAS':['FGOALS-f3-L'],  'CCCma':['CanESM5'],  'CMCC':['CMCC-CM2-SR5'],  \
                'CNRM-CERFACS':['CNRM-ESM2-1'],  'DWD':['MPI-ESM1-2-LR'],  
                'EC-Earth-Consortium':['EC-Earth3'],  'IPSL':['IPSL-CM6A-LR'],  \
                'MIROC':['MIROC6'], 'MOHC':['HadGEM3-GC31-MM'],  'MPI-M':['MPI-ESM1-2-HR'],  \
                'MRI':['MRI-ESM2-0'],  'NCAR':['CESM1-1-CAM5-CMIP5'],  'NCC':['NorCPM1']}

for model in listModel:
    for version in versionList[model]:
        print(version)
        filout='/modfs/project/EUCP/ralkama/CMIP6/DCPP/NAO/EOF_Matched/ATMOSindexBoxLatest_%s_%s.txt'%(version,season)
        if not os.path.exists(filout):
            df=[]
            chemin='/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/'%(model,version)
            repertoires = [d.name for d in Path(chemin).iterdir() if d.is_dir()] # liste des repertoires
            sorted_list = sorted(repertoires, key=lambda x: int(x[1:5]))
            chemin='/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/%s/Amon/tas/'%(model,version,sorted_list[0])
            repertoires = [d.name for d in Path(chemin).iterdir() if d.is_dir()] 
            if 'gr' in repertoires:
                grid = 'gr'
            elif 'gn' in repertoires:
                grid = 'gn'
            else:
                print('le type de grille gr et gn n existe pas pour le model %s version %s'%(model,version))
                sys.exit()
            for decad in sorted_list:
                match = re.match(r's(\d+)-r(\d+)i(\d+)p(\d+)f(\d+)', decad)
                yr, run, ini, member, forcing = map(int, match.groups())
                print (yr,end='  ')
                chemin='/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/%s/Amon/psl/%s/' %(model,version,decad,grid)
                if os.path.exists(chemin):
                    reps= [d.name for d in Path(chemin).iterdir() if d.is_dir()]
                    sorted_reps = sorted(reps, key=sort_key, reverse=True)
                    fichier=glob.glob('/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/%s/Amon/psl/%s/%s/'\
                                    %(model,version,decad,grid,sorted_reps[0])+'*.nc') 
                    if len(fichier)==0 and len(reps)>=2:
                        fichier=glob.glob('/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/%s/Amon/psl/%s/%s/'\
                                    %(model,version,decad,grid,sorted_reps[1])+'*.nc') 
                    if len(fichier)==0 and len(reps)>=3:
                        fichier=glob.glob('/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/%s/Amon/psl/%s/%s/'\
                                    %(model,version,decad,grid,sorted_reps[2])+'*.nc') 
                    if len(fichier)==0 and len(reps)>=4:
                        fichier=glob.glob('/bdd/CMIP6/DCPP/%s/%s/dcppA-hindcast/%s/Amon/psl/%s/%s/'\
                                    %(model,version,decad,grid,sorted_reps[3])+'*.nc') 
                    ...
                    if 'time' in NAO.coords: 
                        ...
                        df_tmp['syear']      = [df_tmp['Year'].iloc[0]]*(len(df_tmp))
                    else:    
                        df_tmp = NAO['year'].to_dataframe(name='Year')
                        df_tmp['syear']      = [df_tmp['Year'].iloc[0]]*(len(df_tmp))
                    if season == 'monthly':
                        df_tmp['month'] = NAO['time.month'].to_dataframe()['month']
                        if df_tmp['month'].iloc[0] == 1 : 
                            df_tmp['LeadTime'] = df_tmp['Year']-df_tmp['Year'].iloc[0]+1
                        else:
                            df_tmp['LeadTime'] = df_tmp['Year']-df_tmp['Year'].iloc[0]
                    else:
                        df_tmp['LeadTime'] = range(1, len(df_tmp)+1, 1)
            ...
            if season == 'monthly':
                df.reset_index().to_csv(filout,sep=' ', columns=['Year', 'month', 'run', 'ini','member', 'forcing', 'syear', 'LeadTime', 'NAO', 'WEPA', 'EAP', 'PNA', 'WPO','TNH','NA','EP','SOI','SCAND','MedSCAND','WEPAbox','NAOi'], header=True, index=False)
            else:
                df.reset_index().to_csv(filout,sep=' ', columns=['Year', 'run', 'ini','member', 'forcing', 'syear', 'LeadTime', 'NAO', 'WEPA', 'EAP', 'PNA', 'WPO','TNH','NA','EP','SOI','SCAND','MedSCAND','WEPAbox','NAOi'], header=True, index=False)
